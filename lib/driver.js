...
function openUrl(context, script, callback) {
  ...
  var title = (browser.name || browser.browserName || browser.device);
  var title   = title + ' ' + (browser.version || '*');
...
function connectBrowser(context, callback) {
  var caps = context.browser;
  if(!caps) {
      throw new Error('context.browser not provided', context);
  }
